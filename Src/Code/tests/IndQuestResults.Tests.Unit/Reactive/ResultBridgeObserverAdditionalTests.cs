...
public class ResultBridgeObserverAdditionalTests
{
    ...
    public void OnNext_ExceptionInHandler_PreservesException()
    {
        ...
        var obs = new IndQuestResults.Reactive.ResultBridgeObserver<int>(
            r =>
            {
                // Simulate the downstream handler throwing while processing a success value;
                // the bridge must catch it and re-deliver a failure carrying the exception,
                // which this handler then records.
                if (r.IsSuccess)
                {
                    throw exception;
                }
